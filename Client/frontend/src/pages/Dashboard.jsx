...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, completed: 0, reading: 0, tbr: 0 })
  ...
  useEffect(() => {
    async function load() {
      try {
        const [books, statsData] = await Promise.all([bookService.getAll(), bookService.getStats()])
        setActivity(books.slice(0, 8))
        setStats(statsData)
      } catch {
        // If no books yet, seed them
        try {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const newBook = await bookService.create({
        ...
        status: form.status, rating: form.rating, notes: form.notes,
        progress: form.status === 'Completed' ? 100 : form.status === 'Reading' ? 50 : 0,
      })
          ...
          {activity.map(book => (
            <div key={book.id} className="col-md-6 col-lg-3 fade-in-up">
              <div className="card card-glass h-100 border-0">
                {book.coverUrl && (
                  <img src={book.coverUrl} className="card-img-top" alt={book.title} style={{ height: 250, objectFit: 'cover' }} />
                )}
